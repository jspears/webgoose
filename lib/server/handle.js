var mquery = require('mongoose/node_modules/mquery'), isFunction = require('lodash/lang/isFunction'), pick = require('lodash/object/pick');
        ...
        if (qp) {
            ...
            if (isMerge(ret)) {
                ...
                Object.keys(qp).forEach(function (key) {
                    if (key === '_conditions' || key === 'op' || key === 'options') {
                        return;
                    }
                    var rkey = key.replace(/^_/, '');
                    if (typeof ret[rkey] === 'function') {
                        ret[rkey](qp[key]);
                    }
                }, ret);

                //            var Query = mquery(qp._conditions, pick(qp, '_distinct', 'limit'), ret.collection, ret);
                return ret.merge(mquery(qp._conditions, qp.options)).exec(send);
            }
            if (isPromise(ret)) {
                return ret.then(function (o) {
